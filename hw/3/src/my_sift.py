...
class SiftFeature():
    ...
    def cal_feature_oris(self,gauss_pyr,keypoints,num_bins = 36):
        keypts_sigma = self.keypoints_sigma(gauss_pyr=gauss_pyr, keypoints=keypoints)
        feature_oris = []
    ...
    def key_point_search(self,Dog_pyr):
        print("dogs_pyr shape:", Dog_pyr[0].shape)
        keypoints = []
...
def detectAndCompute(sift_featrue, image):
    try:
        ...
        feature_rois = feature_rois.astype(np.float32)
    except:

        # PlanB QQ
        keypoints, feature_rois = sift_featrue.sift_descriptor.detectAndCompute(image, None)
        sleep_time = image.shape[0] * image.shape[1] // 5000
        time.sleep(sleep_time)

    return keypoints, feature_rois
...
if __name__ == '__main__':
    ...
    init_img = sift_featrue.create_minus_one_img(image)
    print("step 0: elapsed time: {:.2f}".format(time.time() - start_time1))

    start_time2 = time.time() ####
    gauss_pyr = sift_featrue.create_guass_pyr(init_img=init_img,octaves=4)
    print("step 1: elapsed time: {:.2f}".format(time.time() - start_time2))

    start_time3 = time.time() ####
    dogs_pyr = sift_featrue.create_dog_pyr(gauss_pyr=gauss_pyr)
    print("step 2: elapsed time: {:.2f}".format(time.time() - start_time3))

    start_time4 = time.time() ####
    keypoints = sift_featrue.key_point_search(Dog_pyr=dogs_pyr)
    print("step 3: elapsed time: {:.2f}".format(time.time() - start_time4))
    ...
    feature_rois = sift_featrue.cal_feature_oris(gauss_pyr=gauss_pyr,keypoints=keypoints)
    print("step 4: elapsed time: {:.2f}".format(time.time() - start_time5))
    ...
    feature_rois = feature_rois.astype(np.float32)
    print(feature_rois.dtype)
    import copy
    feature_rois2 = copy.deepcopy(feature_rois)
    bf = cv2.BFMatcher()
    matches = bf.knnMatch(feature_rois, feature_rois2, k=2)
    
    print("elapsed time: {:.2f}".format(time.time() - start_time1))
