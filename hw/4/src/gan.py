...
def inference(opt):
    ...
    Tensor = torch.cuda.FloatTensor if cuda else torch.FloatTensor
    z = Variable(
        Tensor(np.random.normal(0, 1, (int(opt.inference_num/5), opt.latent_dim))))
    
    for seq in range(5):
        gen_imgs = generator(z)

        for i in tqdm(range(0, int(opt.inference_num/5))):
            save_image(gen_imgs.data[i],
                        "result/images_inference/%d.png" % (seq*int(opt.inference_num/5)+i),
                        normalize=True)
